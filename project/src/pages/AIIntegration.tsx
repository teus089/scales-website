import { Link } from 'react-router-dom';
import { Brain, ArrowRight, Check } from 'lucide-react';

export default function AIIntegration() {
  return (
    <div className="bg-white">
      <section className="relative bg-gradient-to-br from-slate-900 to-slate-800 text-white overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 via-transparent to-blue-500/10"></div>
        <div className="relative mx-auto max-w-7xl px-6 py-20 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <div className="inline-flex items-center gap-2 mb-6">
              <Brain className="h-8 w-8 text-cyan-300" />
              <span className="text-sm font-bold text-cyan-300 uppercase tracking-wider">Service</span>
            </div>
            <h1 className="text-4xl sm:text-6xl font-bold tracking-tight leading-[1.1] mb-6">
              AI Integration
            </h1>
            <p className="text-lg sm:text-xl text-neutral-300 leading-relaxed">
              Identify high-impact AI opportunities and implement solutions that deliver measurable results without the hype.
            </p>
          </div>
        </div>
      </section>

      <section className="bg-white">
        <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <div className="space-y-8">
              <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-neutral-900 mb-6">
                  AI without the hype
                </h2>
                <p className="text-base sm:text-lg text-neutral-700 leading-relaxed mb-4">
                  Strategic AI integration focuses on concrete business value. I help you cut through the noise and identify where AI can actually move the needle for your business.
                </p>
                <p className="text-base sm:text-lg text-neutral-700 leading-relaxed">
                  You get reduced costs, improved decision-making speed, and unlocked capabilities that weren't previously feasible. No buzzwords, just practical implementations that deliver ROI.
                </p>
              </div>

              <div className="bg-gradient-to-br from-cyan-50 to-white p-8 border-l-4 border-cyan-500">
                <h3 className="text-2xl font-bold text-neutral-900 mb-4">What you get</h3>
                <ul className="space-y-3">
                  {[
                    'Comprehensive AI opportunity assessment',
                    'Custom AI model development and training',
                    'Integration with existing systems and workflows',
                    'Natural language processing solutions',
                    'Predictive analytics and forecasting',
                    'Automated decision-making systems',
                    'Performance monitoring and optimization',
                    'Team training and documentation'
                  ].map((item) => (
                    <li key={item} className="flex items-start gap-3">
                      <Check className="h-5 w-5 text-cyan-500 flex-shrink-0 mt-0.5" />
                      <span className="text-neutral-700">{item}</span>
                    </li>
                  ))}
                </ul>
              </div>

              <div>
                <h3 className="text-2xl font-bold text-neutral-900 mb-4">Who this is for</h3>
                <p className="text-base sm:text-lg text-neutral-700 leading-relaxed mb-4">
                  Perfect for businesses serious about AI but skeptical of overhyped solutions. You want results, not experimental technology.
                </p>
                <p className="text-base sm:text-lg text-neutral-700 leading-relaxed">
                  Ideal for companies with clear processes that could benefit from intelligent automation, and organizations ready to make strategic investments in transformative technology.
                </p>
              </div>
            </div>
          </div>
        </div>
      </section>

      <section className="bg-gradient-to-br from-cyan-50 to-white py-16">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-5xl">
            <h2 className="text-3xl font-bold text-neutral-900 mb-3">Pricing</h2>
            <p className="text-neutral-700 mb-8">
              I'll build custom AI integrations that deliver measurable results for your business. The one-time fee covers building and deploying your system. Monthly pricing is for ongoing maintenance, support, and improvements.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white border-2 border-neutral-200 p-6 hover:border-cyan-500 transition-all">
                <h3 className="text-lg font-bold text-neutral-900 mb-2">Starter</h3>
                <p className="text-3xl font-bold text-neutral-900 mb-6">$1,500 + $250/month</p>
                <ul className="space-y-3">
                  <li className="text-sm text-neutral-700">• Build 1–2 core AI integrations</li>
                  <li className="text-sm text-neutral-700">• Initial testing and deployment</li>
                  <li className="text-sm text-neutral-700">• Monitoring 1–2 integrations</li>
                  <li className="text-sm text-neutral-700">• Bug fixes and maintenance</li>
                  <li className="text-sm text-neutral-700">• Email support</li>
                </ul>
              </div>
              <div className="bg-white border-2 border-neutral-200 p-6 hover:border-cyan-500 transition-all">
                <h3 className="text-lg font-bold text-neutral-900 mb-2">Professional</h3>
                <p className="text-3xl font-bold text-neutral-900 mb-6">$3,000 + $500/month</p>
                <ul className="space-y-3">
                  <li className="text-sm text-neutral-700">• Multiple connected AI systems</li>
                  <li className="text-sm text-neutral-700">• Custom model training</li>
                  <li className="text-sm text-neutral-700">• Support for multiple integrations</li>
                  <li className="text-sm text-neutral-700">• Monthly tweaks or improvements</li>
                  <li className="text-sm text-neutral-700">• Faster response time</li>
                </ul>
              </div>
              <div className="bg-white border-2 border-neutral-200 p-6 hover:border-cyan-500 transition-all">
                <h3 className="text-lg font-bold text-neutral-900 mb-2">Premium</h3>
                <p className="text-3xl font-bold text-neutral-900 mb-6">$5,000 + $1,000/month</p>
                <ul className="space-y-3">
                  <li className="text-sm text-neutral-700">• Custom AI system architecture</li>
                  <li className="text-sm text-neutral-700">• Advanced model development</li>
                  <li className="text-sm text-neutral-700">• Ongoing optimization</li>
                  <li className="text-sm text-neutral-700">• New capabilities added over time</li>
                  <li className="text-sm text-neutral-700">• Priority support and reviews</li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </section>

      <section className="relative bg-gradient-to-br from-cyan-50 to-blue-50 overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-200/30 rounded-full blur-3xl"></div>
        <div className="relative mx-auto max-w-7xl px-6 py-16 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-neutral-900 mb-4">
              Ready to leverage AI strategically?
            </h2>
            <p className="text-base sm:text-lg text-neutral-700 leading-relaxed mb-8">
              Let's explore how AI can drive real value in your business.
            </p>
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 px-6 py-3 bg-neutral-900 text-white text-sm font-semibold hover:bg-neutral-800 transition-all hover:scale-105"
            >
              Get started
              <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
      </section>
    </div>
  );
}
